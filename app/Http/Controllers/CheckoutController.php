...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            if (auth()->check()) {
                ...
                if ($isAgentMember) {
                    foreach ($carts as $cartItem) {
                        $product = Product::find($cartItem['product_id']);
                        if ($product->agent_discount > 0) {
                            $price = cart_product_price($cartItem, $product, false, false);
                            $agent_discount_total += ($price * $product->agent_discount / 100) * $cartItem['quantity'];
                        }
                    }
              ...
              $bulk_cod_pay_amount = $total;
                ...
                if (auth()->user() && auth()->user()->is_bulk_buyer == "1") {
                    $is_bulk_buyer = true;
            ...
            return view('frontend.checkout', compact('carts', 'address_id', 'total', 'carrier_list', 'shipping_info','is_bulk_buyer','bulk_online_pay_amount','bulk_cod_pay_amount', 'agent_discount_total'));
        }
    ...
    public function checkout(Request $request)
    {
        ...
        //----  ----//
        $has_bulk_quantity = false;

        foreach ($carts as $cart) {
            if ($cart->quantity > 1) {
                $has_bulk_quantity = true;
                break;
            }
        }
        //---- End ----//
    ...
    public function checkout_done($combined_order_id, $payment)
    {
        ...
        foreach ($combined_order->orders as $key => $order) {
            ...
            if ($order->is_split_payment) {
                // Only mark advance payment as paid
                $order->advance_payment_status = 'paid';
                $order->advance_payment_details = $payment;
                $order->advance_paid_at = now();
                $order->payment_status = 'partial_paid';
                
                // Email notification for advance payment
                EmailUtility::order_email($order, 'partial_paid');
        ...
        // Clear bulk buyer sessions
        Session::forget(['is_bulk_buyer_split_payment', 'advance_amount', 'cod_amount', 'original_payment_option']);
    ...
    public function apply_coupon_code(Request $request)
    {
        ...
        $returnHTML = view(
        'frontend.partials.cart.cart_summary',compact('coupon','carts','proceed'))->render();
...
public function toggleOnlinePay(Request $request)
{
    if (!auth()->check()) {
        return response()->json(['success' => false]);
    }

    $value = (int) $request->is_online_pay; // 0 or 1
